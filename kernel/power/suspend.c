...
#if defined(CONFIG_ARCH_ACER_MSM8974)
static time_t fd_time = 0;
static bool enter_fd = false;
fast_dormancy_level fast_dormancy_enabled = FD_PDP_DEACTIVATE;
#endif
...
void kernel_enter_fast_dormancy(void)
{
	...
	if (!enter_fd) {
		...
		printk(KERN_INFO "kernel_enter_fast_dormancy Enter\n");
	}
...
#if defined(CONFIG_ARCH_ACER_MSM8974)
	if(strcmp(annotation, "entry") == 0)
	{
		if(ts.tv_sec > (fd_time + 3))
		{
			fd_time = ts.tv_sec;
			enter_fd = true;
		}
