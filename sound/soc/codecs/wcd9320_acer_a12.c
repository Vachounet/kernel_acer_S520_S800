...
struct snd_soc_codec *acer_codec;
unsigned int wcd9320_get_chip_version(void);
static int wcd9320_open(struct inode *inode, struct file *file);
static int wcd9320_close(struct inode *inode, struct file *file);
static long wcd9320_ioctl(struct file *file, unsigned int cmd, unsigned long arg);
#endif

#ifdef CONFIG_ACER_HEADSET
struct platform_device *acer_taiko_pdev;

extern struct snd_soc_jack hs_jack;
extern struct snd_soc_jack button_jack;

void acer_headset_jack_report(int headset_type);
...
#ifdef CONFIG_ACER_HEADSET_BUTT
void press_headset_button(int button_num);
#endif
...
unsigned int wcd9320_get_chip_version(void)
{
	...
	value = snd_soc_read(acer_codec, TAIKO_A_CHIP_VERSION);

	return (value & 0x0F);
}

static const struct file_operations wcd9320_fops = {
	.owner			= THIS_MODULE,
	.open			= wcd9320_open,
	.release		= wcd9320_close,
	.unlocked_ioctl	= wcd9320_ioctl,
};

static struct miscdevice wcd9320_dev = {
	.minor	= MISC_DYNAMIC_MINOR,
	.name	= WCD9320_DRIVER_NAME,
	.fops	= &wcd9320_fops,
};

static struct kobject *audio_dev_info_kobj = NULL;
...
static ssize_t codec_show(struct kobject *kobj, struct kobj_attribute *attr, char * buf)
{
	...
	ver = wcd9320_get_chip_version();
	s += sprintf(s, "wcd9320, version=%i\n", ver);
	return (s - buf);
}
...
static ssize_t dolby_show(struct kobject *kobj, struct kobj_attribute *attr, char * buf)
{
	char *s = buf;

	s += sprintf(s, "Dolby DS1\n");
	return (s - buf);
}
...
static int wcd9320_open(struct inode *inode, struct file *file)
{
	pr_debug("[WCD9320] has been opened\n");
	return 0;
}

static int wcd9320_close(struct inode *inode, struct file *file)
{
	pr_debug("[WCD9320] has been closed\n");
	return 0;
}
...
static long wcd9320_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	pr_debug("[WCD9320] wcd9320 ioctl \n");
	if (_IOC_TYPE(cmd) != WCD9320_IOCTL_MAGIC) {
		pr_err("[WCD9320] IOCTL: cmd magic type error\n");
		return -ENOTTY;
	}

	if (_IOC_NR(cmd) > IOC_MAXNR) {
		pr_err("[WCD9320] IOCTL: cmd number error\n");
		return -ENOTTY;
	}
	...
	if (err) {
		pr_err("[WCD9320] IOCTL: cmd access right error\n");
		return -EFAULT;
	}

	switch(cmd) {
	case WCD9320_SET_CODEC: {
		unsigned int reg;
		unsigned int value;
		struct wcd9320_codec_info codec_info;

		printk("AUDIO_SET_CODEC...\r\n");

		if (copy_from_user(&codec_info, (void __user *)arg, sizeof(struct wcd9320_codec_info)))
			return -1;

		reg = 0xffff & codec_info.reg;
		value = 0xffff & codec_info.val;

		if (0 != snd_soc_write(acer_codec, reg, value))
			return -1;

		return 0;
	}

	case WCD9320_GET_CODEC: {
		unsigned int reg;
		unsigned int value;
		struct wcd9320_codec_info codec_info;
		...
		reg = 0xffff & codec_info.reg;
		value = snd_soc_read(acer_codec, reg);

		codec_info.reg = reg;
		codec_info.val = value;

		printk("AUDIO_GET_CODEC : reg=%x, val=%x...\r\n", codec_info.reg, codec_info.val);

		if (copy_to_user((void __user *) arg, &codec_info, sizeof(struct wcd9320_codec_info)))
			return -1;
	...
	default:
		pr_err("[WCD9320] IOCTL: Command not found!\n");
		return -1;
	}
}
#endif
...
#ifdef CONFIG_ACER_HEADSET
struct taiko_priv *acer_taiko_priv;

void acer_headset_jack_report(int headset_type)
{
	switch(headset_type) {
	case 0:
		snd_soc_jack_report_no_dapm(&hs_jack,
									0,
									TAIKO_JACK_MASK);
		break;
...
void press_headset_button(int button_num)
{
	...
	if (button_num == 0) {
		button_press = SND_JACK_BTN_0;

		snd_soc_jack_report(&button_jack, button_press, button_press);
		do {
			mdelay(10);
			get_mpp_hs_mic_adc(&adc_reply);
		} while (adc_reply < 0x8000);
		snd_soc_jack_report(&button_jack, 0, button_press);
	}
}
#endif
...
static int taiko_handle_pdata(struct taiko_priv *taiko)
{
...
#ifdef CONFIG_MACH_ACER_A12
	snd_soc_write(codec,  TAIKO_A_MICB_CFILT_3_VAL, 0x98);
#else
	...
	value = (pdata->micbias.bias3_cap_mode == MICBIAS_EXT_BYP_CAP ?
		 0x00 : 0x16);
#ifdef CONFIG_MACH_ACER_A12
	snd_soc_write(codec,  TAIKO_A_MICB_3_CTL, 0xDA);
#else
...
static const struct wcd9xxx_reg_mask_val taiko_2_0_reg_defaults[] = {
	...
	TAIKO_REG_VAL(TAIKO_A_RX_HPH_BIAS_PA, 0x7A),
#ifdef CONFIG_MACH_ACER_A12
	TAIKO_REG_VAL(TAIKO_A_RX_HPH_OCP_CTL, 0x6A),
#else
...
static int taiko_codec_probe(struct snd_soc_codec *codec)
{
	...
	ret = sysfs_create_group(audio_dev_info_kobj, &attr_group);
	if (ret) {
...
{
#ifdef CONFIG_MACH_ACER_A12
	if (misc_register(&wcd9320_dev)) {
		pr_err("taiko_codec_init: wcd9320 device register failed\n");
		return -ENOMEM;
	}
#endif
