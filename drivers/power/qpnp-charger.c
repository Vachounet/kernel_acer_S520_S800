...
struct qpnp_chg_chip {
	...
	struct delayed_work		power_key_work;
#endif
...
#ifdef CONFIG_MACH_ACER_A12
		/* if USB current bigger than 1000mA, show charging(AC) */
		if (acer_boot_mode != CHARGER_BOOT) {
			if (qpnp_chg_is_usb_chg_plugged_in(chip)) {
				union power_supply_propval ret = {0,};
				chip->usb_psy->get_property(chip->usb_psy,
							POWER_SUPPLY_PROP_CURRENT_MAX, &ret);
				if ((ret.intval / 1000) >= 1000)
					val->intval = true;
			}
		}
#else
...
extern void as3668_heat_indicator(int heat_num);
#endif
static int
get_prop_batt_status(struct qpnp_chg_chip *chip)
{
	...
	if (chip->bat_is_cool || chip->bat_is_warm ) {
		if (qpnp_chg_is_usb_chg_plugged_in(chip)) {
			/* set LED to static blue when over temperature */
			as3668_heat_indicator(BATT_OVER_HEAT);
		} else {
			as3668_heat_indicator(BATT_UNDER_HEAT);
		}
	} else {
		as3668_heat_indicator(BATT_UNDER_HEAT);
	}
...
int get_real_batt_status(void)
{
	...
	rc = qpnp_chg_read(the_chip, &chgr_sts, INT_RT_STS(the_chip->chgr_base), 1);
	if (rc) {
		pr_err("failed to read interrupt sts %d\n", rc);
		return POWER_SUPPLY_CHARGE_TYPE_NONE;
	}
...
EXPORT_SYMBOL(get_real_batt_status);
#endif
...
#ifdef CONFIG_MACH_ACER_A12
#define FULL_PERCENTAGE 98
static bool is_bat_full = false;
static int adjust_cap(int cap)
{
	int cap_offset = 100 - FULL_PERCENTAGE;

	if (cap >= FULL_PERCENTAGE) {
		is_bat_full = true;
		return 100;
	} else {
		is_bat_full = false;
		if (cap > 75)
			return cap + cap_offset;
		else if (cap > 65 && cap_offset > 1)
			return cap + cap_offset - 1;
		else if (cap > 55 && cap_offset > 2)
			return cap + cap_offset - 2;
		else if (cap > 45 && cap_offset > 3)
			return cap + cap_offset - 3;
		else if (cap > 35 && cap_offset > 4)
			return cap + cap_offset - 4;
		else if (cap > 25 && cap_offset > 5)
			return cap + cap_offset - 5;
		else
			return cap;
	}
}
...
#ifdef CONFIG_MACH_ACER_A12
		if (qpnp_chg_is_usb_chg_plugged_in(chip) && is_bat_full)
...
#ifdef CONFIG_MACH_ACER_A12
		val->intval = adjust_cap(get_prop_capacity(chip));
#else
...
#ifdef CONFIG_MACH_ACER_A12
static void
qpnp_chg_set_appropriate_battery_charge(struct qpnp_chg_chip *chip)
{
	if (chip->bat_is_cool || chip->bat_is_warm) {
		pr_info("disable\n");
		...
		if (!delayed_work_pending(&chip->power_key_work)) {
			pr_info("enable\n");
...
#ifdef CONFIG_MACH_ACER_A12
#define CONSECUTIVE_COUNT	12  //120s
#else
...
#ifdef CONFIG_MACH_ACER_A12
	/* Disable charging if battery over temperature */
	if (chip->bat_is_cool || chip->bat_is_warm)
		qpnp_chg_force_run_on_batt(chip, 0);
#endif
...
#ifdef CONFIG_MACH_ACER_A12
		/* Workaround for Bug32180 */
		if (!chip->resuming_charging && !is_bat_full
			&& !(chip->bat_is_cool | chip->bat_is_warm)
			&& (chg_sts & VBAT_DET_LOW_IRQ)) {
			chip->resuming_charging = true;
			qpnp_chg_set_appropriate_vbatdet(chip);
			qpnp_chg_charge_en(chip, !chip->charging_disabled);
			pr_info("not full, resuming charging\n");
		} else
#endif
			goto stop_eoc;
	}
...
#ifdef CONFIG_MACH_ACER_A12
void qpnp_batt_chg_enable(int enable)
{
	...
	ret.intval = enable;
	bat_psy->set_property(bat_psy,
			POWER_SUPPLY_PROP_CHARGING_ENABLED, &ret);
	usb_psy->set_property(usb_psy,
			POWER_SUPPLY_PROP_PRESENT, &ret);
}
EXPORT_SYMBOL(qpnp_batt_chg_enable);
#endif

#ifdef CONFIG_MACH_ACER_A12
#define HYSTERISIS_DECIDEGC 50
#else
...
qpnp_chg_adc_notification(enum qpnp_tm_state state, void *ctx)
{
	...
	if (chip->bat_is_cool ^ bat_cool || chip->bat_is_warm ^ bat_warm) {
		...
		qpnp_chg_set_appropriate_battery_current(chip);
#ifdef CONFIG_MACH_ACER_A12
		qpnp_chg_set_appropriate_battery_charge(chip);
#endif
		qpnp_chg_set_appropriate_vbatdet(chip);
	}
...
#ifdef CONFIG_MACH_ACER_A12
/* BMS parameters */
int bms_fcc, bms_uuc, bms_ruc, bms_rc, bms_rbat, bms_soc, bms_energy;
/* dev-info_battery sysfs */
static struct kobject *qpnp_chg_dbg_kobj;
static ssize_t Vendor_show(struct kobject *kobj,
				struct kobj_attribute *attr, char * buf)
{
	return sprintf(buf, "LG\n");
}

static ssize_t Charge_full_design_show(struct kobject *kobj,
				struct kobj_attribute *attr, char * buf)
{
	return sprintf(buf, "3200mAh\n");
}
...
static ssize_t FCC_show(struct kobject *kobj,
				struct kobj_attribute *attr, char * buf)
{
	return sprintf(buf, "%d\n", bms_fcc);
}

static ssize_t UUC_show(struct kobject *kobj,
				struct kobj_attribute *attr, char * buf)
{
	return sprintf(buf, "%d\n", bms_uuc);
}

static ssize_t RUC_show(struct kobject *kobj,
				struct kobj_attribute *attr, char * buf)
{
	return sprintf(buf, "%d\n", bms_ruc);
}

static ssize_t RC_show(struct kobject *kobj,
				struct kobj_attribute *attr, char * buf)
{
	return sprintf(buf, "%d\n", bms_rc);
}

static ssize_t RBAT_show(struct kobject *kobj,
				struct kobj_attribute *attr, char * buf)
{
	return sprintf(buf, "%d\n", bms_rbat);
}

static ssize_t SoC_show(struct kobject *kobj,
				struct kobj_attribute *attr, char * buf)
{
	return sprintf(buf, "%d\n", bms_soc);
}

static ssize_t ENERGY_show(struct kobject *kobj,
				struct kobj_attribute *attr, char * buf)
{
	return sprintf(buf, "%d\n", bms_energy);
}

/* charging control sysfs */
static ssize_t ctl_vbus_on_show(struct kobject *kobj,
				struct kobj_attribute *attr, char * buf)
{
	qpnp_chg_charge_en(the_chip, 1);
	qpnp_chg_force_run_on_batt(the_chip, 0);

	return sprintf(buf, "on\n");
}

static ssize_t ctl_vbus_off_show(struct kobject *kobj,
				struct kobj_attribute *attr, char * buf)
{
	qpnp_chg_charge_en(the_chip, 0);
	qpnp_chg_force_run_on_batt(the_chip, 1);

	return sprintf(buf, "off\n");
}

static ssize_t ctl_chg_on_show(struct kobject *kobj,
				struct kobj_attribute *attr, char * buf)
{
	the_chip->charging_disabled = false;
	qpnp_chg_charge_en(the_chip, !the_chip->charging_disabled);
	qpnp_chg_force_run_on_batt(the_chip, 0);

	return sprintf(buf, "on\n");
}

static ssize_t ctl_chg_off_show(struct kobject *kobj,
				struct kobj_attribute *attr, char * buf)
{
...
#define ctl_debug_attr_show(_name) \
	static struct kobj_attribute _name##_attr = { \
	.attr = { \
	.name = __stringify(_name), \
	.mode = 0640, \
...
debug_attr_show(Vendor);
debug_attr_show(Charge_full_design);
debug_attr_show(Firmware);
debug_attr_show(FCC);
debug_attr_show(UUC);
debug_attr_show(RUC);
debug_attr_show(RC);
debug_attr_show(RBAT);
debug_attr_show(SoC);
debug_attr_show(ENERGY);
debug_attr_show(ctl_vbus_on);
debug_attr_show(ctl_vbus_off);
debug_attr_show(ctl_chg_on);
debug_attr_show(ctl_chg_off);
...
static struct attribute * attrs[] = {
	...
	&SoC_attr.attr,
	&ENERGY_attr.attr,
	&ctl_vbus_on_attr.attr,
	&ctl_vbus_off_attr.attr,
	&ctl_chg_on_attr.attr,
	&ctl_chg_off_attr.attr,
	NULL,
};
...
#ifdef CONFIG_MACH_ACER_A12
		/* Set battery voltage threshold for trickle charging to 2400mV */
		qpnp_chg_masked_write(chip, chip->chgr_base + CHGR_VBAT_TRKL,
			0x0F, 0x07, 1);
		/* Set weak battery voltage threshold to 2800mV */
		qpnp_chg_masked_write(chip, chip->chgr_base + CHGR_VBAT_WEAK,
			0x0F, 0x07, 1);
#endif
		/* HACK: Disable wdog */
...
#ifdef CONFIG_MACH_ACER_A12
extern int qpnp_pon_is_pwrkey_press(void);
#define POWER_KEY_CHECK_MS 6000
static void
qpnp_power_key_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct qpnp_chg_chip *chip = container_of(dwork,
				struct qpnp_chg_chip, power_key_work);

	if (chip->bat_is_cool || chip->bat_is_warm) {
		if (qpnp_pon_is_pwrkey_press() == 1) {
			pr_info("over temperature\n");
			schedule_delayed_work(&chip->power_key_work,
						msecs_to_jiffies(POWER_KEY_CHECK_MS));
		}
		return;
	}

	if (qpnp_pon_is_pwrkey_press() == 1) {
		pr_info("power key is pressed\n");
		if (!chip->charging_disabled) {
			chip->charging_disabled = true;
			qpnp_chg_charge_en(chip, !chip->charging_disabled);
			qpnp_chg_force_run_on_batt(chip, 0);
...
void qpnp_chg_pwrkey_press(void)
{
	if (the_chip == NULL)
		return;

	if (!delayed_work_pending(&the_chip->power_key_work))
		schedule_delayed_work(&the_chip->power_key_work,
					msecs_to_jiffies(POWER_KEY_CHECK_MS));
}
EXPORT_SYMBOL(qpnp_chg_pwrkey_press);
#endif
...
#ifdef CONFIG_MACH_ACER_A12
	wake_lock_init(&chip->arb_wake_lock,
		WAKE_LOCK_SUSPEND, "qpnp-chg-arb-lock");
#endif
...
#ifdef CONFIG_MACH_ACER_A12
	INIT_DELAYED_WORK(&chip->power_key_work, qpnp_power_key_work);
	if (qpnp_pon_is_pwrkey_press() == 1) {
		pr_info("power key is pressed\n");
		chip->charging_disabled = true;
		qpnp_chg_charge_en(chip, !chip->charging_disabled);
...
#ifdef CONFIG_MACH_ACER_A12
	the_chip = chip;
	/* Create dev-info_battery sysfs */
	qpnp_chg_dbg_kobj = kobject_create_and_add("dev-info_battery", NULL);
	if (qpnp_chg_dbg_kobj == NULL) {
		pr_err("kobject_create_and_add dev-info_battery failed\n");
	}
	rc = sysfs_create_group(qpnp_chg_dbg_kobj, &dbg_attr_group);
	if (rc) {
...
#ifdef CONFIG_MACH_ACER_A12
static void qpnp_chg_shutdown(struct spmi_device *spmi)
{
	struct qpnp_chg_chip *chip = dev_get_drvdata(&spmi->dev);
	...
	qpnp_chg_charge_en(chip, 0);
	qpnp_chg_force_run_on_batt(chip, 0);
	pr_info("disable battery charging\n");
}
#endif
