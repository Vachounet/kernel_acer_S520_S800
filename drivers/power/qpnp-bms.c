...
#ifdef CONFIG_MACH_ACER_A12
extern int get_real_batt_status(void);
#endif
static int get_battery_status(struct qpnp_bms_chip *chip)
{
#ifdef CONFIG_MACH_ACER_A12
	return get_real_batt_status();
#else
...
#ifdef CONFIG_MACH_ACER_A12
extern int bms_fcc, bms_uuc, bms_ruc, bms_rc, bms_rbat, bms_soc;
#endif
...
#ifdef CONFIG_MACH_ACER_A12
	bms_fcc = params.fcc_uah;
	bms_uuc = params.uuc_uah;
	bms_ruc = remaining_usable_charge_uah;
	bms_rc = params.ocv_charge_uah;
	bms_rbat = params.rbatt_mohm;
	bms_soc = soc;
#endif
...
#ifdef CONFIG_MACH_ACER_A12
	} else if (chip->batt_type == BATT_A12) {
		batt_data = &a12_3200_data;
		pr_info("load a12 battery profile\n");
#endif
	} else {
