...
#define DWC3_CHG_DCD_MAX_RETRIES	10 /* Tdcd_tmout = 10 * 100 msec */
	...
	case USB_CHG_STATE_DCD_DONE:
		...
		if (vout) {
			...
			if (!dcd && prop_chg_detect) {
				...
				/*
				 * Check unknow and usb charger status
				 * Unknow charger => DWC3_DCP_CARGER
				 * USB => DWC3_SDP_CHARGER
				*/
				if(dcd)
