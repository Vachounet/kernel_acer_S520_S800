...
#ifdef CONFIG_MACH_ACER_A12
			if (retry_times < MAX_REG_RETRY_TIMES) {
				pr_info("vbus_dwc3 regulator retry times = %d\n",retry_times+1);
				msleep(100);
				retry_times++;
				goto RETRY_REG;
			}
...
#ifdef CONFIG_MACH_ACER_A12
static unsigned int mhl_chg_type = POWER_SUPPLY_TYPE_BATTERY;  //Battery
void dwc3_mhl_charger_type_set(unsigned int chg_type)
{
	pr_debug("MHL chg_type=%d\n", chg_type);
	mhl_chg_type = chg_type;
}
#endif
...
#ifdef CONFIG_MACH_ACER_A12
	if (mhl_chg_type != POWER_SUPPLY_TYPE_BATTERY) {
		if (mhl_chg_type == POWER_SUPPLY_TYPE_USB)  //USB
			mA = DWC3_IDEV_CHG_MIN;
		else if (mhl_chg_type == POWER_SUPPLY_TYPE_MAINS)  //AC
		...
		dev_info(phy->dev, "Avail curr from MHL = %u\n", mA);
...
#ifdef CONFIG_MACH_ACER_A12
					/*
                                         * For USB charger plug-in charger only mode issue.
                                         */
