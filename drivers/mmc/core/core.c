...
#if defined(CONFIG_MACH_ACER_A12)
		if (host->index == 1 && cd && gpio_is_valid(cd->gpio)) {
			if (!gpio_get_value(cd->gpio)) {
				printk("sd card is inserted and init SD fail\n");
				host->init_fail = true;
			}
		}
#endif
					...
					unsigned long mode, void *unused)
{
	...
	case PM_POST_RESTORE:
		...
		spin_unlock_irqrestore(&host->lock, flags);
#if defined(CONFIG_MACH_ACER_A12)
		if (host->pending_resume && (mmc_card_sd(host->card))) {
			printk("mmc_detect_change for SD delay 600ms\n");
			mmc_detect_change(host, msecs_to_jiffies(600));
		} else {
			if (host->init_fail == false) {
				mmc_detect_change(host, 0);
			} else
				printk("skip sd mmc_detect_change\n");
		}
#else
		mmc_detect_change(host, 0);
#endif
