...
extern int cmd_debug_mask;
static struct kobject *device_info_kobj = NULL;
#define SD_MAX_LEN	8
static char speed_class[SD_MAX_LEN + 1] = "Reserved";
static char grade_speed_uhs[SD_MAX_LEN + 1] = "Reserved";
static char sd_mode[SD_MAX_LEN + 1] = "Reserved";
...
debug_attr(SPEED_CLASS, "%s\n", speed_class);
debug_attr(GRADE_SPEED_UHS, "%s\n", grade_speed_uhs);
debug_attr(SD_MODE, "%s\n", sd_mode);

static struct attribute * group[] = {
	&SPEED_CLASS_attr.attr,
	&GRADE_SPEED_UHS_attr.attr,
	&SD_MODE_attr.attr,
	NULL,
};
...
#if defined(CONFIG_MACH_ACER_A12)
static void info_to_speed(int sc, int gsu)
{
	switch(sc) {
	case 0:
	...
	case 4:
		...
		break;
	default:
		sprintf(speed_class, "Reserved");
		break;
	}
	if (gsu == 0)
		sprintf(grade_speed_uhs, "<10MB/s");
	else if (gsu == 1)
		sprintf(grade_speed_uhs, ">=10MB/s");
	else
		sprintf(grade_speed_uhs, "Reserved");
}
#endif
...
static int mmc_read_ssr(struct mmc_card *card)
{
	...
	gsu = UNSTUFF_BITS(ssr, 396 - 384, 4);
	info_to_speed(sc, gsu);
#endif
out:
	kfree(ssr);
	return err;
	...
	struct mmc_card *oldcard)
{
	...
	if (device_info_kobj == NULL) {
		device_info_kobj = kobject_create_and_add("dev-info_sd", NULL);
		if (device_info_kobj == NULL) {
			printk("%s: dev-info_SD register failed\n", mmc_hostname(card->host));
		} else {
...
static void mmc_sd_remove(struct mmc_host *host)
{
	...
	mmc_release_host(host);
#if defined(CONFIG_MACH_ACER_A12)
	kobject_del(device_info_kobj);
	device_info_kobj = NULL;
#endif
}
