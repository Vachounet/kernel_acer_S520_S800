...
static void sdhci_send_command(struct sdhci_host *host, struct mmc_command *cmd)
{
	...
	SDHCI_TRACE(host, "%lld: %s: %s: updated 0x8=0x%08x 0xC=0x%08x 0xE=0x%08x\n",
			ktime_to_ms(ktime_get()), __func__,
			mmc_hostname(host->mmc),
			sdhci_readl(host, SDHCI_ARGUMENT),
...
#if defined(CONFIG_MACH_ACER_A12)
	if (mmc->pending_resume && (mmc_card_sd(host->mmc->card))) {
		mmc->pending_resume = false;
		printk("==>   now start run %s mmc_resume_host\n", mmc_hostname(mmc));
		mmc_resume_host(mmc);
	}
#endif
...
static void sdhci_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	unsigned long flags;
#if defined(CONFIG_MACH_ACER_A12)
	struct mmc_cd_gpio *cd = mmc->hotplug.handler_priv;
#endif
	...
	if (host->quirks & SDHCI_QUIRK_BROKEN_CARD_DETECTION) {
		...
		if (cd && gpio_is_valid(cd->gpio))
			present = !gpio_get_value(cd->gpio);
		else
			present = true;
#else
		present = true;
	...
	if (!present || host->flags & SDHCI_DEVICE_DEAD) {
		...
		if ((host->flags & SDHCI_NEEDS_RETUNING) &&
			...
			while (retry > 0) {
				spin_unlock_irqrestore(&host->lock, flags);
				err = sdhci_execute_tuning(mmc, mrq->cmd->opcode);
				spin_lock_irqsave(&host->lock, flags);
				...
				if (err) {
					retry = retry -1;
					printk("%s - %s: tuning failed,retry=%d\n",
						mmc_hostname(mmc),__func__,retry);
				} else {
					break;
				}
			}
			if (err) {
				printk("tuning fail in %s \n",__func__);
				BUG_ON(err);
			}
		}
...
static void sdhci_cmd_irq(struct sdhci_host *host, u32 intmask)
{
	...
	SDHCI_TRACE(host, "%lld: %s: %s: resp 0: 0x%08x resp 1: 0x%08x resp 2: 0x%08x resp 3: 0x%08x\n",
					ktime_to_ms(ktime_get()), __func__,
					mmc_hostname(host->mmc),
					sdhci_readl(host, SDHCI_RESPONSE),
					sdhci_readl(host, SDHCI_RESPONSE + 0x4),
